        ...
        <script>
            ...
            function startCamera() {
                ...
                if (location.protocol !== 'https:' && location.hostname !== 'localhost' && location.hostname !== '127.0.0.1') {
                    Swal.fire({
                        icon: 'error',
                        title: 'Akses Ditolak Browser',
                        html: 'Browser memblokir akses kamera karena koneksi <b>Tidak Aman (HTTP)</b>.<br><br>Untuk menggunakan kamera di HP, Anda wajib menggunakan <b>HTTPS</b> (misal: pakai Ngrok/Valet Share) atau akses via Localhost.',
                        footer: '<a href="https://developer.mozilla.org/en-US/docs/Web/Security/Secure_Contexts" target="_blank">Baca Selengkapnya</a>'
                    });
                    return;
                }
                ...
                if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
                    Swal.fire({
                        ...
                        text: 'Browser atau perangkat Anda tidak mendukung akses kamera web.',
                    });
                    return;
                }
                ...
                document.getElementById('scanStatus').innerText = "Meminta izin kamera...";
                ...
                html5QrCode.start({ facingMode: "environment" }, config, onScanSuccess, onScanFailure)
                    ...
                    .catch(err => {
                        ...
                        if (err.name == 'NotAllowedError' || err.name == 'PermissionDeniedError') {
                            ...
                            errorMsg = "Kamera sedang digunakan oleh aplikasi lain atau tidak bisa diakses.";
                        }

                        // Fallback to any camera if environment failed
                        html5QrCode.start({}, config, onScanSuccess, onScanFailure)
                            .then(() => {
                                isCameraRunning = true;
                                document.getElementById('scanStatus').innerText = "Kamera Aktif (Mode Depan/Default)";
                                const btn = document.getElementById('toggleCameraBtn');
                                btn.innerHTML = '<i class="bi bi-camera-video-off me-2"></i> Matikan Kamera';
                                btn.classList.replace('btn-outline-primary', 'btn-primary');
                                ...
                                Swal.fire({
                                    icon: 'info',
                                    title: 'Kamera Belakang Gagal',
                                    text: 'Gagal membuka kamera belakang, beralih ke kamera default.',
                                    timer: 3000
                                });
                            })
                            .catch(err2 => {
                                document.getElementById('scanStatus').innerText = "Error: " + err.name;
